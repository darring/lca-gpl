...
_run_cmd() {
 ...
 wget -O /tmp/laf-testing2 "NMSA_IP/laf_update.php?sid=$SID&type=1&data=`echo $full_cmd |sed -e 's/ /+/g'`" &> /tmp/laf-testing
 http_proxy=$tmp_proxy
 $command > $cmd_output_f
 if [ $report_bck -eq 1 ]; then
  tmp_proxy=$http_proxy
  http_proxy=""
  wget -O /tmp/laf-testing3 "NMSA_IP/laf_update.php?sid=$SID&type=2&data=`cat $cmd_output_f|sed -e 's/ /+/g'`" &> /tmp/laf-testing4
  http_proxy=$tmp_proxy
 fi 
