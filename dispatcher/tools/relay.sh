...
function update_sids {
 for sid in `ls $SID_DIR`; do
  UPDATE_URL="http://nmsa01/client_push.php?mac=00:1B:21:44:F0:C4&sid=$sid&comp=1&log="
  clear_proxy
...
function command_query {
 QUERY_URL="http://nmsa01/client_poll.php?mac=00:1B:21:44:F0:C4&hostname=$HOSTNAME"
 ...
 if ! grep nothing /tmp/query.url &> /dev/null;then 
  ...
  ret=$?
  UPDATE_URL="http://nmsa01/client_push.php?mac=00:1B:21:44:F0:C4&sid=$sid&comp=$ret&log=relay.sh-`date "+%m-%d-%y-%H_%M"|sed -e 's/ /+/g' `-`cat $OUTPUT_DIR/$sid`"
  clear_proxy
